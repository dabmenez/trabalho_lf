#!/usr/bin/env python3
"""
Teste da MTU com lógica corrigida para número par de 'a's
"""
...
def main():
    print("=== TESTE DA MTU COM LÓGICA CORRIGIDA ===\n")
    
    print("1. TESTE: MT CORRIGIDA para número par de 'a's")
    print("-" * 50)
    
    
    print("\n🔧 CORREÇÃO APLICADA:")
    print("- q1 (par): s->s,R,q1 | a1->a1,R,q11 | b->b,R,qf")
    print("- q11 (ímpar): a1->a1,R,q1 | b->b,R,q111 (rejeição)")
    print("- q111 (rejeição): qualquer símbolo permanece em q111")
    ...
    print("Teste 1.4: Entrada 'saaa' (3 a's - deve rejeitar)")
    entrada1_4 = mt1_codigo + "$saaa"
    resultado1_4 = simular_mtu(entrada1_4)
    ...
    print(f"MT1 + 'saaa' (3 a's): {resultado1_4.split('#')[-1] if resultado1_4 else 'ERRO'} {'✓' if resultado1_4 and resultado1_4.endswith('#R') else '✗'}")
    
    testes_corretos = (
        resultado1_1 and resultado1_1.endswith('#A') and  # MT1 deve aceitar número par de a's (2)
        resultado1_2 and resultado1_2.endswith('#R') and  # MT1 deve rejeitar número ímpar de a's (1)
        resultado1_3 and resultado1_3.endswith('#A') and  # MT1 deve aceitar 0 a's (par)
        resultado1_4 and resultado1_4.endswith('#R')      # MT1 deve rejeitar número ímpar de a's (3)
    )
    ...
    if testes_corretos:
        ...
        print("✅ Lógica de estados par/ímpar implementada corretamente")
    else:
